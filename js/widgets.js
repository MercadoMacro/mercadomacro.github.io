...
function renderMarketOverviewWidget(theme, targetContainerId = 'market-overview-widget-wrapper') {
    ...
    if (!container) { console.error(`Market Overview widget container com ID "${targetContainerId}" não encontrado.`); if (targetContainerId === 'modal-market-overview-container' && modalContentArea) { modalContentArea.innerHTML = `<p class="error-commentary" style="padding:20px; text-align:center;"><i class="fas fa-exclamation-triangle"></i> Container do widget de mercado não encontrado no modal.</p>`; } return; }
    const skeleton = container.querySelector('.tv-skeleton');
    if (skeleton) skeleton.style.display = 'none';
    let oldTvWidgetDiv = container.querySelector('.tradingview-widget-container');
    while (oldTvWidgetDiv) { oldTvWidgetDiv.remove(); oldTvWidgetDiv = container.querySelector('.tradingview-widget-container'); }
...
function loadEconomicCalendarWidget() {
    ...
    const configJsonString = JSON.stringify({ "width": "100%", "height": "100%", "mode": "1", "theme": widgetTheme, "lang": "pt" });
