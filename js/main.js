...
function updateCommentary(content) {
    ...
    const formattedContent = content
        ...
        .replace(/<br><br>/g, '</div><div class="commentary-paragraph">')
        // Correção para listas
        .replace(/<li>(.+?)<br>/g, '<li>$1</li>');
    
    commentary.innerHTML = `
        ${bionicToggleContainer}
    ...
    commentary.dataset.originalHtml = commentary.querySelector('.commentary-content').innerHTML;
...
function toggleBionicReading() {
    ...
    const contentElement = commentary.querySelector('.commentary-content');
    
    if (isActive) {
        // Armazena o HTML original se ainda não estiver armazenado
        if (!commentary.dataset.originalHtml) {
            commentary.dataset.originalHtml = contentElement.innerHTML;
        }
        
        // Aplica Bionic Reading preservando a estrutura HTML
        const bionicContent = applyBionicReading(contentElement.textContent);
        contentElement.innerHTML = bionicContent;
        
        // Atualiza o ícone do botão
        document.querySelector('#bionic-toggle i').className = 'fas fa-eye-slash';
    } else {
        // Restaura o conteúdo original
        if (commentary.dataset.originalHtml) {
            contentElement.innerHTML = commentary.dataset.originalHtml;
        }
        // Atualiza o ícone do botão
        document.querySelector('#bionic-toggle i').className = 'fas fa-eye';
    }
}

function applyBionicReading(text, fixation = 3) {
    // Preserva tags HTML e aplica Bionic apenas no texto
    return text.replace(/[^\s<>]+|(<[^>]+>)/g, (match) => {
        // Ignora tags HTML
        if (match.startsWith('<')) return match;
        
        // Aplica Bionic em palavras com mais de 3 caracteres
        if (match.length > fixation) {
            const boldPart = match.substring(0, fixation);
            const rest = match.substring(fixation);
            return `<strong class="bionic-word">${boldPart}</strong>${rest}`;
        }
        return match;
    });
}
